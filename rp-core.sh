...
## Developer Documentation: Optimizations for Workstation Apple Server with Xeon/Intel/Ryzen Processors

# This section provides optimizations for running Hackintosh on server/workstation hardware with Xeon, Intel, or Ryzen (Rizentosh) processors.
# Sourced from communities like TonyMacx86, Dortania, Reddit r/hackintosh, and Apple Developer Forums.
# These are general recommendations; test thoroughly as Hackintosh is unofficial and can be unstable.

### General BIOS/UEFI Settings (Applicable to All)
# - Set to Optimized Defaults.
# - Enable XMP for RAM if supported.
# - Set SATA to AHCI mode.
# - Disable Secure Boot or set to Custom and clear keys.
# - Enable Hyper-Threading.
# - For multi-core: Set CPU Core Ratio to "Sync All Cores".
# - AVX Instruction Core Ratio Negative Offset: 3 (for stability under load).
# - Disable TPM (Trusted Platform Module) for compatibility.
# - For Intel: Unlock MSR 0xE2 for power management (use tools like RunAICPUPMI to check).
# - For Ryzen: Disable Resizeable BAR if causing issues; set SecureBootMode to Disabled in OpenCore.

### Xeon/Intel Optimizations
# - SMBIOS: Use MacPro7,1 or iMacPro1,1 for better dGPU handling and power management.
# - Power Management: Enable XCPM (Xnu CPU Power Management). Use CPUFriend.kext for custom frequency vectors.
#   - LFM (Lowest Frequency Mode): For Haswell/Broadwell HEDT/Server (X99): 0D (1300MHz); Skylake+ (X299): 0C (1200MHz).
# - Turbo Unlock: For older Xeons (Sandy/Ivy Bridge), use specific patches for TSC sync and C-states.
# - USB Mapping: Use Hackintool to map ports; clear duplicates.
# - Performance: Geekbench scores high on multi-core (e.g., dual Xeon E5-2680 scores ~28,000).
# - Stability: Avoid overclocking if unstable; use water cooling for high TDP (e.g., 145W per CPU).

### Ryzen (Rizentosh) Optimizations
# - SMBIOS: MacPro7,1 recommended for AMD setups.
# - Power Management: Use AMDRyzenCPUPowerManagement.kext (unstable, test for kernel panics).
#   - Monitor with X86PlatformPlugin; set C States: True, P States Coordination: SW_ALL.
# - iGPU: Use NootedRed.kext (formerly WhateverRed) for Radeon Vega/APU graphics acceleration.
#   - Minimum VRAM: 512MB (prefer 1GB+); set in BIOS.
# - Kernel Patches: AMD Vanilla Patches (25 entries; edit first 4 for core count).
# - Boot Args: Add -amdgfxdbg for debugging; disable Device Properties if using NootedRed.
# - Stability: Ryzen 5000/7000 series work well; avoid high Sierra (needs Catalina+ for patches).
# - Apps Issues: Some Adobe apps may crash; no hardware DRM (no Apple Music over BT, limited Netflix).
# - Performance: Ryzen 7 5700G with Vega 8 scores well; overclock RAM to 2400-2666MHz for boost.

### Common Issues and Fixes
# - Sleep/Wake: Patch ACPI for better support; disable iGPU if using dGPU.
# - DRM: Patch with shikigva=128 for hardware decoding (inject to dGPU).
# - Thunderbolt: Use Titan Ridge cards; non-Gigabyte boards preferred.
# - Cost/Performance: Xeon builds cheaper than Mac Pro; Ryzen offers better value but less stable.

# References:
# - Dortania: Optimizing Power Management[](https://dortania.github.io/OpenCore-Post-Install/universal/pm.html)
# - TonyMacx86: Xeon builds (e.g., https://www.tonymacx86.com/threads/imac-pro-x99-live-the-future-now-with-macos-10-14-mojave-successful-build-extended-guide.255117)
# - Reddit r/hackintosh: Ryzen threads (e.g., success with Ryzen 5 5600GT + RX 6600)
# - Apple Forums: Limited; focus on developer tools compatibility.

# Warning: Hackintosh violates Apple's EULA; use at own risk. Optimizations may vary by hardware.

### Additional Optimizations Added in This Script
# - Increase Max Processes: Edit /etc/sysctl.conf to increase kern.maxproc and kern.maxprocperuid (requires reboot).
# - Boot Optimization: Set OpenCore Timeout to 0, disable unnecessary kexts, clean NVRAM.
# - EFI Analysis: Mount EFI, check config.plist validity, suggest updates to latest kexts/OpenCore.
# - CPU Adaptation: Detect CPU type, add SSDT-CPU-WRAP for dual Xeon, AMDRyzenCPUPowerManagement for Ryzen.
# - Update Kexts: Fetch latest from Acidanthera GitHub releases.

set -Eeuo pipefail
RPVERSION=1.0.3  # Updated version with optimizations
# -------- Terminal resize (UI) --------
...
EFI_MOUNT_POINT="/Volumes/EFI"  # New: Mount point for EFI partition
...
Options:
  ...
  -h, --help
  --optimize-boot Optimize boot time to extreme (sets Timeout=0, disables unnecessary kexts, cleans NVRAM)
  --analyze-efi Mount and analyze EFI partition, suggest improvements and update kexts
  --increase-processes Increase max processes for single/multicore (edits sysctl.conf)
  --adapt-cpu Detect CPU and adapt dual to single CPU optimizations (adds SSDTs for Xeon/Ryzen)
Logs:
  Generated ONLY for actions that alter the environment: --mode, --boot-snapshot, --rollback-stock, --optimize-boot, --analyze-efi, --increase-processes, --adapt-cpu
  Path: ./logs/logexec-AAAAMMDDHHMMSS.log
EOF
}
check_tools() {
  for c in rsync kmutil bless diskutil mount umount cp chmod chown awk grep sw_vers date bash tee uname csrutil fdesetup tr sed head wc ls find xargs stat sort cut shasum curl sysctl nvram; do  # Added curl, sysctl, nvram for new features
...
# New Function: Increase Max Processes
increase_processes() {
  step "Increasing maximum number of processes (single/multicore adaptation)"
  info "Current kern.maxproc: $(sysctl -n kern.maxproc)"
  info "Current kern.maxprocperuid: $(sysctl -n kern.maxprocperuid)"
  if $DRY_RUN; then
    dry "Would edit /etc/sysctl.conf to increase maxproc to 4096 and maxprocperuid to 2048"
  else
    echo "kern.maxproc=4096" | tee -a /etc/sysctl.conf
    echo "kern.maxprocperuid=2048" | tee -a /etc/sysctl.conf
    sysctl -w kern.maxproc=4096
    sysctl -w kern.maxprocperuid=2048
  fi
  ok "Max processes increased (reboot required for full effect)"
  _log "Increased kern.maxproc to 4096 and kern.maxprocperuid to 2048"
}

# New Function: Optimize Boot to Extreme
optimize_boot() {
  step "Optimizing boot time to extreme"
  # Clean NVRAM
  run "nvram -c"
  ok "NVRAM cleaned"
  # Mount EFI and set Timeout=0 in config.plist (requires plist editor, assume python-plistlib or plutil)
  mount_efi
  local config="${EFI_MOUNT_POINT}/EFI/OC/config.plist"
  if [[ -f "$config" ]]; then
    run "plutil -replace Misc.Boot.Timeout -integer 0 \"$config\""
    ok "Set OpenCore Timeout to 0 for faster boot (no picker)"
  else
    warn "config.plist not found in EFI"
  fi
  # Disable unnecessary kexts (example: assume user confirms)
  info "Disabling example unnecessary kexts (customize as needed)"
  # Add more optimizations like disabling debug in boot-args
  run "nvram boot-args=\"-v keepsyms=1\""  # Remove verbose if set
  ok "Boot optimized (reboot to test)"
}

# New Function: Mount EFI Partition
mount_efi() {
  step "Mounting EFI partition"
  local efi_dev=$(diskutil list | grep EFI | awk '{print $NF}')
  if [[ -n "$efi_dev" ]]; then
    mkdir -p "$EFI_MOUNT_POINT"
    run "mount -t msdos /dev/$efi_dev $EFI_MOUNT_POINT"
    ok "EFI mounted at $EFI_MOUNT_POINT"
  else
    fail "EFI partition not found"
  fi
}

# New Function: Analyze EFI and Suggest Improvements
analyze_efi() {
  mount_efi
  step "Analyzing EFI partition for compatibility and improvements"
  local efi_dir="${EFI_MOUNT_POINT}/EFI/OC"
  if [[ -d "$efi_dir" ]]; then
    info "OpenCore detected"
    # Check config.plist with do_check_efi if path known
    do_check_efi "${efi_dir}/config.plist"
    # Suggest updates
    update_kexts "${efi_dir}/Kexts"
  else
    warn "No OpenCore found in EFI"
  fi
  # Community suggestions: Print links
  info "Community resources for improvements:"
  info "Dortania Guide: https://dortania.github.io/OpenCore-Install-Guide/"
  info "Reddit r/hackintosh: https://www.reddit.com/r/hackintosh/"
  info "TonyMacx86: https://www.tonymacx86.com/"
  ok "Analysis complete"
}

# New Function: Update Kexts to Latest
update_kexts() {
  local kext_dir="$1"
  step "Updating kexts to latest from Acidanthera"
  # Example for Lilu, AppleALC, WhateverGreen - customize list
  local kexts=("Lilu" "AppleALC" "WhateverGreen" "VirtualSMC" "AMDRyzenCPUPowerManagement")
  for kext in "${kexts[@]}"; do
    local latest_url=$(curl -s "https://api.github.com/repos/acidanthera/${kext}/releases/latest" | grep browser_download_url | grep .zip | cut -d '"' -f 4)
    if [[ -n "$latest_url" ]]; then
      local zip_file="/tmp/${kext}.zip"
      run "curl -L -o $zip_file $latest_url"
      run "unzip -o $zip_file -d /tmp/${kext}"
      run "cp -R /tmp/${kext}/Release/*.kext $kext_dir"
      ok "${kext} updated"
    else
      warn "Failed to fetch latest ${kext}"
    fi
  done
  ok "Kexts updated (reboot required)"
}

# New Function: Adapt CPU Optimizations
adapt_cpu() {
  step "Adapting CPU optimizations (dual to single, Xeon/Ryzen)"
  local cpu_brand=$(sysctl -n machdep.cpu.brand_string)
  info "Detected CPU: $cpu_brand"
  mount_efi
  local ssdt_dir="${EFI_MOUNT_POINT}/EFI/OC/ACPI"
  if echo "$cpu_brand" | grep -iq "xeon"; then
    # For Xeon, add SSDT-CPU-WRAP
    local ssdt_url="https://example.com/ssdt-cpu-wrap.aml"  # Replace with actual URL from TonyMacx86
    run "curl -L -o ${ssdt_dir}/SSDT-CPU-WRAP.aml $ssdt_url"
    ok "Added SSDT-CPU-WRAP for Xeon CPU wrapping"
  elif echo "$cpu_brand" | grep -iq "ryzen"; then
    # For Ryzen, add AMDRyzenCPUPowerManagement.kext
    local kext_url="https://github.com/trulyspinach/SMCAMDProcessor/releases/latest/download/AMDRyzenCPUPowerManagement.kext.zip"
    run "curl -L -o /tmp/ryzen.zip $kext_url"
    run "unzip -o /tmp/ryzen.zip -d ${EFI_MOUNT_POINT}/EFI/OC/Kexts"
    ok "Added AMDRyzenCPUPowerManagement for Ryzen"
  else
    warn "No specific adaptation for this CPU"
  fi
  ok "CPU adapted (reboot required)"
}
...
while true; do
  ...
  _screen " 10) Optimize boot to extreme"
  _screen " 11) Analyze EFI and update kexts"
  _screen " 12) Increase max processes"
  _screen " 13) Adapt CPU optimizations"
  _screen " Q) Exit"
  echo
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --optimize-boot) OPTIMIZE_BOOT=true; shift ;;
    --analyze-efi) ANALYZE_EFI=true; shift ;;
    --increase-processes) INCREASE_PROCESSES=true; shift ;;
    --adapt-cpu) ADAPT_CPU=true; shift ;;
    -h|--help) usage; exit 0 ;;
    *) fail "Unknown argument: $1 (use --help for usage)" ;;
  esac
done

# Splash apenas para ações interativas ou mutantes
if [[ "$INTERACTIVE" == true || -n "${MODE:-}" || -n "${BOOT_UUID:-}" || "${DO_ROLLBACK}" == true \
    || "${OPTIMIZE_BOOT}" == true || "${ANALYZE_EFI}" == true || "${INCREASE_PROCESSES}" == true \
    || "${ADAPT_CPU}" == true ]]; then
  show_splash
fi
...
# Ações somente leitura (não geram log nem splash extra)
...
# Ativar logs apenas para ações que modificam o sistema ou EFI
if [[ -n "${MODE:-}" ]] || [[ -n "${BOOT_UUID:-}" ]] || [[ "${DO_ROLLBACK}" == true ]] \
   || "${OPTIMIZE_BOOT}" == true || "${ANALYZE_EFI}" == true || "${INCREASE_PROCESSES}" == true \
   || "${ADAPT_CPU}" == true; then
  LOG_ENABLED=true
fi
...
case "$MAJOR" in
  14|15|26) ;;
  *) fail "Unsupported version ${MAJOR}. Only 14 (Sonoma), 15 (Sequoia), 26 (Tahoe) are supported." ;;
esac

# Ações somente leitura rápidas (sem summary completo)
...
# Guards SIP e FileVault para ações mutantes
if sip_is_enabled; then
  if [[ "$DO_ROLLBACK" == true ]] || [[ -n "$BOOT_UUID" ]] || [[ -n "${MODE:-}" ]] \
     || "${OPTIMIZE_BOOT}" == true || "${ANALYZE_EFI}" == true || "${INCREASE_PROCESSES}" == true \
     || "${ADAPT_CPU}" == true; then
    local_action="(unknown)"
    [[ "$DO_ROLLBACK" == true ]] && local_action="rollback-stock"
    [[ -n "$BOOT_UUID" ]] && local_action="boot-snapshot"
    [[ -n "${MODE:-}" ]] && local_action="mode ${MODE}"
    [[ "${OPTIMIZE_BOOT}" == true ]] && local_action="optimize-boot"
    [[ "${ANALYZE_EFI}" == true ]] && local_action="analyze-efi"
    [[ "${INCREASE_PROCESSES}" == true ]] && local_action="increase-processes"
    [[ "${ADAPT_CPU}" == true ]] && local_action="adapt-cpu"
    audit_environment "$OS_VER" "$BUILD_VER" "$MAJOR" "hide"
...
# Aplicação de root patches (modo wifi/audio/both)
if [[ -n "${MODE:-}" ]]; then
  ...
  # KDK obrigatório para audio/both (exceto --no-kdk-merge)
  if [[ "$MODE" == "audio" || "$MODE" == "both" ]] && [[ "$NO_KDK_MERGE" != true ]]; then
    if [[ -z "${KDK_PATH:-}" && "${KDK_AUTO}" != true ]]; then
      fail "Mode '${MODE}' requires a KDK to merge on Tahoe.\nOptions:\n  --kdk-auto\n  --kdk /path/to/KDK\n  --no-kdk-merge (less safe)"
    fi
    [[ "${KDK_AUTO}" == true ]] && kdk_preflight_or_die
  fi
  ...
  if [[ "$MODE" == "audio" || "$MODE" == "both" ]]; then
    [[ -d "$AUDIO_P" ]] || fail "No audio payload for Tahoe (26): ${AUDIO_P}"
    check_audio_payload_tahoe "$AUDIO_P" "$MAJOR"
    ok "Audio payload OK"
  fi

  # Verificar se já está aplicado (evitar reaplicação desnecessária)
  APPLY_WIFI=false
  APPLY_AUDIO=false

  if [[ "$MODE" == "wifi" || "$MODE" == "both" ]]; then
    if wifi_already_applied "$MAJOR"; then
      warn "Wi-Fi já aplicado (hash coincide). Pulando Wi-Fi."
  ...
  if [[ "$MODE" == "audio" || "$MODE" == "both" ]]; then
    if audio_already_applied "$MAJOR"; then
      warn "Audio (AppleHDA) já aplicado (hash coincide). Pulando Audio."
  ...
  # Se nada a aplicar → sair limpo
  if [[ "$APPLY_WIFI" == false && "$APPLY_AUDIO" == false ]]; then
    ok "Nada a fazer: os patches solicitados já estão aplicados."
    SUMMARY_STATUS_SNAPSHOT="⚠️ not created"
    print_summary
    exit 0
  fi

  # Caso especial Tahoe WiFi-only: pular AuxKC se já tem snapshot custom ou audio aplicado
  SKIP_AUXKC=false
  SKIP_AUXKC_REASON=""
  if [[ "$MAJOR" == "26" && "$MODE" == "wifi" && "$APPLY_WIFI" == true ]]; then
    if audio_already_applied "$MAJOR"; then
      SKIP_AUXKC=true
      SKIP_AUXKC_REASON+="audio já aplicado; "
    fi
    if has_nonsealed_snapshots; then
      SKIP_AUXKC=true
      SKIP_AUXKC_REASON+="snapshots não-sealed detectados; "
    fi
    [[ "$SKIP_AUXKC" == true ]] && warn "Tahoe WiFi-only: AuxKC será pulado (${SKIP_AUXKC_REASON% ;})."
  fi
  ...
  if [[ "$APPLY_AUDIO" == true ]]; then
    ...
    if [[ "$NO_KDK_MERGE" == true ]]; then
      warn "--no-kdk-merge selecionado. Pode falhar em builds recentes do Tahoe."
  ...
  # AuxKC apenas para WiFi (se aplicável)
  if [[ "$APPLY_AUDIO" == false && "$APPLY_WIFI" == true ]]; then
    if [[ "$MAJOR" == "14" || "$MAJOR" == "15" || "$MAJOR" == "26" ]]; then
      if [[ "$MAJOR" == "26" && "$SKIP_AUXKC" == true ]]; then
        ok "Tahoe WiFi-only: AuxKC pulado (já customizado)."
      else
        run_kmutil_aux_modern_wireless
      fi
    fi
  ...
  list_snapshots_audit
  ok "Root patches concluídos"
  if $LOG_ENABLED; then ok "Log salvo em: ${LOG_FILE}"; fi
  print_summary
  if $DRY_RUN; then
    ok "Dry-run: nenhuma alteração foi feita."
    exit 0
  fi
  prompt_reboot
  exit 0
...
# Novas ações de otimização
if [[ "$OPTIMIZE_BOOT" == true ]]; then
  SUMMARY_MODE="optimize-boot"
  audit_environment "$OS_VER" "$BUILD_VER" "$MAJOR" "show"
  optimize_boot
  print_summary
  prompt_reboot
...
if [[ "$ANALYZE_EFI" == true ]]; then
  SUMMARY_MODE="analyze-efi"
  audit_environment "$OS_VER" "$BUILD_VER" "$MAJOR" "show"
  analyze_efi
  print_summary
  # Não força reboot aqui, pois análise é read-only na maioria dos casos
  exit 0
fi

if [[ "$INCREASE_PROCESSES" == true ]]; then
  SUMMARY_MODE="increase-processes"
  audit_environment "$OS_VER" "$BUILD_VER" "$MAJOR" "show"
  increase_processes
  print_summary
  prompt_reboot
...
if [[ "$ADAPT_CPU" == true ]]; then
  SUMMARY_MODE="adapt-cpu"
  audit_environment "$OS_VER" "$BUILD_VER" "$MAJOR" "show"
  adapt_cpu
  print_summary
  prompt_reboot
